...
def system_prompt(*, tenant_id: str, as_of: str) -> str:
    return (
        ...
        "When they ask to download or export a CSV, call the matching locked tool (or pass "
        "source=completes_by_provider|ar_past_30_days|referrals) then export_csv. "
        "Put the returned url in the answer as a raw path like /api/exports/<id>.csv "
        "(not markdown) so the UI can make it a download link.\n"
