...
def system_prompt(*, tenant_id: str, as_of: str) -> str:
    return (
        ...
        "When they ask to download or export a file, call the matching locked tool "
        "(completes_by_provider with months=N for Completes / closed appointments by provider) "
        "then export_table (format=xlsx for excel/xlsx/spreadsheet; format=csv or export_csv "
        "when they say CSV). After any export_* tool, ALWAYS emit a short prose reply that "
        "includes the raw url (/api/exports/<id>.xlsx or .csv), not markdown.\n"
